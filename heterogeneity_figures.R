...
## LRT P_int: 1-df likelihood ratio test (moderator as main effect in f0)
...
run_city_het <- function(diff_var, mean_var, rand_part, point_color, outfile) {
  ...
  p_int_df <- tibble(
    ...
    p_int = c(
      pint_lrt(dat, diff_var, mean_var, "city_pop_density_log",  ctrl_rhs, rand_part),
      pint_lrt(dat, diff_var, mean_var, "city_gdp_log",          ctrl_rhs, rand_part),
      pint_lrt(dat, diff_var, mean_var, "city_pop_density_2022", ctrl_rhs, rand_part)
    )
  )
...
run_city_het("tree_diff", "tree_mean", "(1 + tree_diff | participant_id)",
             "#0B6623",
             file.path(out_dir, "figure5_tree_city_heterogeneity.png"))

run_city_het("wl_diff", "wl_mean", "(1 + wl_diff | participant_id)",
             "#B45309",
             file.path(out_dir, "figure6_wildlife_city_heterogeneity.png"))

run_city_het("wa_diff", "wa_mean", "(1 + wa_diff | participant_id)",
             "#1E3A8A",
             file.path(out_dir, "figure7_water_city_heterogeneity.png"))
...
run_mh_het("tree_diff", "tree_mean", "(1 + tree_diff | participant_id)",
           "#0B6623",
           file.path(out_dir, "figure8_tree_mentalhealth_heterogeneity.png"))

run_mh_het("wl_diff", "wl_mean", "(1 + wl_diff | participant_id)",
           "#B45309",
           file.path(out_dir, "figure9_wildlife_mentalhealth_heterogeneity.png"))

run_mh_het("wa_diff", "wa_mean", "(1 + wa_diff | participant_id)",
           "#1E3A8A",
           file.path(out_dir, "figure10_water_mentalhealth_heterogeneity.png"))
...
## LRT interaction test (1-df): moderator in f0, so Δχ² tests interaction only
...
dat <- dat |>
  mutate(
    ...
    family_wealth = factor(case_when(
      grepl("<\\s*10k|10\\s*[-–]\\s*30k", as.character(income_household), ignore.case=TRUE) ~ "≤30k",
      grepl("30\\s*[-–]\\s*60k",          as.character(income_household), ignore.case=TRUE) ~ "30–60k",
      grepl("60\\s*[-–]\\s*100k|>\\s*100k", as.character(income_household), ignore.case=TRUE) ~ ">60k"
    ), levels = c("≤30k", "30–60k", ">60k")),
...
indiv_mod_order <- c(
  ...
  "Household income", "Individual income (non-students)", "Worktime (non-students)"
)
...
run_indiv_het <- function(diff_var, mean_var, rand_part, point_color, outfile) {
  ...
  res <- bind_rows(
    ...
    fit_subgroup_drop(dat |> filter(!is.na(family_wealth)),
      ...
      mutate(moderator = "Household income"),
  ...
  res <- res |>
    mutate(group = {
      ...
      for (nm in names(label_rename))
        v <- ifelse(toupper(v) == toupper(nm), label_rename[[nm]], v)
      v
    })
